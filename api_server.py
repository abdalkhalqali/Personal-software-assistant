"""
CodeCraft — AI Coding Agent v5.0
Optimized cascade: Groq API → HF API → Tiny Local Model
All three providers are optional, configured via env vars.
"""
import os
import json
import uuid
...
class ChatHandler:
    ...
    def load_local(self):
        ...
        with self.model_lock:
            ...
            import shutil
            # Check if there's enough disk space (~1.5GB needed)
            ...
            try:
                ...
                print(f"[CodeCraft] ⏭️ Local model skipped: {e}")
    ...
    async def chat_stream(self, messages: list, temperature: float, max_tokens: int) -> AsyncGenerator[str, None]:
        ...
        # 1️⃣ Groq Cloud API (fast, high-quality, Llama 70B)
        if GROQ_API_KEY:
        ...
        # 2️⃣ HuggingFace Inference API (good quality, 32B model)
        if HF_TOKEN:
            providers_available += 1
            try:
        ...
        # 3️⃣ Local CPU model (small, slower, last resort)
    ...
    async def _stream_groq(self, messages: list, max_tokens: int) -> AsyncGenerator[str, None]:
        """Stream from Groq Cloud (Llama 70B via OpenAI API)."""
    ...
    async def _stream_local(self, messages: list, temperature: float, max_tokens: int) -> AsyncGenerator[str, None]:
        ...
        def _generate():
            try:
                ...
                gen_kwargs = {**inputs, "max_new_tokens": max_tokens,
                              "temperature": temperature if temperature > 0 else None,
                              "do_sample": temperature > 0, "repetition_penalty": 1.1, "streamer": streamer}
...
async def lifespan(app: FastAPI):
    ...
    print("=" * 60)
    print(f"⚡ Groq:  {'✅ ' + os.environ.get('GROQ_API_KEY','')[:8] + '...' if GROQ_API_KEY else '⏸️  Not set'}")
    print(f"🌐 HF:    {'✅ ' + os.environ.get('HF_TOKEN','')[:8] + '...' if HF_TOKEN else '⏸️  Not set'}")
...
app = FastAPI(title="CodeCraft AI", version="5.0.0", lifespan=lifespan)
...
async def terminal(request: TerminalRequest):
    ...
    try:
        ...
        return {"output": (result.stdout or result.stderr or "✅ Done").strip()}
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for f in filenames:
            ...
            try:
                ...
                files.append({"path": os.path.join(rel, f), "size": size})
            except: pass
...
async def git_commit(request: GitCommitRequest):
    ...
    try:
        ...
        return {"message": f"✅ Committed: {result.stdout.strip()}" if result.returncode == 0 else f"ℹ️ {result.stdout.strip() or result.stderr.strip()}"}
    except Exception as e:
...
async def serve_assets(path: str):
    file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "frontend", "dist", "assets", path)
