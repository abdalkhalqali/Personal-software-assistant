...
وضع التشغيل (محدد تلقائياً):
  1. HF Inference API  → يستخدم HF_TOKEN، مجاني بلا حدود يومية
  2. CPU محلي          → الاحتياط الأخير (يُحمَّل فقط عند فشل الـ API)
"""
import os
import re
import base64
...
# HF Inference API أولاً — يعمل على سيرفرات HF البعيدة
# CPU المحلي يُستخدم فقط كـ fallback إذا فشل الـ API
...
class GitHubClient:
    ...
    def get_file(self, owner: str, repo: str, path: str, branch: str = "main"):
    ...
    def update_file(self, owner: str, repo: str, path: str, branch: str,
                    message: str, content: str, sha=None) -> dict:
...
# ── Model (local CPU — fallback only) ──────────────────────────────────────
model = None
processor = None


def load_model_once():
    """تحميل النموذج محلياً على CPU. يُستدعى فقط إن فشل الـ API."""
...
def _history_to_api(history: list) -> list:
    """
    حوّل تاريخ المحادثة من صيغة Qwen المحلية إلى صيغة OpenAI-chat.
        ...
        for item in content:
            ...
            if t == "text":
                ...
                parts.append({"type": "text",
                               "text": "[فيديو مرفق — غير مدعوم عبر API]"})
        if parts:
...
def _predict_local(chatbot, history):
    """استدلال محلي على CPU — احتياطي إذا فشل API."""
    load_model_once()
...
def predict(chatbot, history):
    """
    المدخل الرئيسي للاستدلال — بدون @spaces.GPU (هذا Space على CPU).
    الأولوية: HF Inference API ← CPU محلي
    """
    if USE_API:
        try:
            ...
            print(f"[Agent] API فشل ({e})، تحويل إلى CPU المحلي…")
            chatbot = list(chatbot or [])
            if chatbot and chatbot[-1].get("role") == "assistant":
                chatbot[-1]["content"] = f"⚠️ API غير متاح، أستخدم CPU محلياً…\n\n"
...
def push_file_to_github(file_path: str, new_content: str, commit_msg: str, proj_state: dict):
    ...
    try:
        ...
        proj_state.get("loaded", {}).pop(file_path, None)
...
with gr.Blocks(
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1, elem_id="sidebar"):
            ...
            gh_token = gr.Textbox(
                label="GitHub Token", placeholder="ghp_xxxxxxxxxxxx",
                type="password", info="مطلوب للمستودعات الخاصة والرفع"
            )
            repo_url = gr.Textbox(
                label="رابط المستودع",
                placeholder="https://github.com/user/repo  أو  user/repo"
            )
            branch_box = gr.Textbox(label="الفرع (Branch)", placeholder="main", value="main")
            ...
            file_picker = gr.Dropdown(label="اختر ملفاً من المستودع", choices=[], interactive=True)
            ...
            with gr.Group(elem_id="push-panel"):
                ...
                push_content = gr.Code(label="محتوى الملف", language="python", lines=8, interactive=True)
                push_msg     = gr.Textbox(label="رسالة الـ Commit", placeholder="إصلاح الخطأ في...")
                push_btn     = gr.Button("⬆️ رفع إلى GitHub", variant="secondary")
                push_status  = gr.Markdown("")

        with gr.Column(scale=3):
            chatbot_ui = gr.Chatbot(
                value=[], label="المحادثة", height=560, type="messages",
                elem_id="chatbot", show_copy_button=True,
            ...
            with gr.Row(elem_id="query-row"):
                query_box = gr.Textbox(
                    ...
                    scale=6, lines=2, max_lines=6, container=False,
                )
                send_btn = gr.Button("إرسال ▶", variant="primary", scale=1, elem_id="send-btn")

            with gr.Row():
                upload_btn = gr.UploadButton("📷 صورة / فيديو", file_types=["image", "video"], scale=2)
...
def main():
    # لا نحمّل النموذج عند الإقلاع — HF API يتولى الاستدلال
    # النموذج المحلي يُحمَّل فقط عند فشل الـ API (lazy loading)
